...
Append age/sex from /results/eeg_exp.csv to:
  - /results/offset_yeo7.csv
  - /results/exponent_yeo7.csv

Rules
  * Merge by Subject (automatically normalize IDs like 'sub1', '001', or 1 -> integer 1).
  * Preserve the original row order and original column order of the target CSVs.
    Only append two new columns at the end: 'age', 'sex'.
  * If a Subject has no match in eeg_exp.csv, age/sex will remain empty (NaN).
"""
...
PATH_OFF  = os.path.join(ROOT, "offset_yeo7.csv")
PATH_EXP  = os.path.join(ROOT, "exponent_yeo7.csv")
# ----------------------------------------------------

def canonical_subject(x):
    """Normalize subject IDs like 'sub12' / '012' / 12 -> 12 (int). Return None if not parseable."""
    if pd.isna(x):
        return None
    s = str(x)
    m = re.search(r"(\d+)", s)
...
def load_meta(path):
    """Read eeg_exp.csv, standardize column names, and return a unique Subject -> (age, sex) table."""
    m = pd.read_csv(path)
    m.columns = [c.strip().lower() for c in m.columns]

    # Flexible column detection: subject, age, sex/gender
    ...
    # Keep the first (age, sex) record per Subject
    m = (
        m.sort_values(["Subject"])
...
def append_meta(in_path, meta_df, out_path=None):
    ...
    Preserves original row/column order; writes back to out_path (defaults to overwrite in_path).
    """
    ...
    # Left merge preserves row order (sort=False)
    ...
    print(f"[OK] Appended age/sex: {out_path} (rows={len(merged)})")
