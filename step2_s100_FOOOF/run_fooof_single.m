...
%      - SFILE         : PSD sFile path (relative to protocol, e.g. subX/.../timefreq_psd_*.mat)
% 3) Optional environment variables:
%      - FOOOF_TAG     : Output comment tag (e.g., "pre-s100-FOOOF" or "post-net-FOOOF")
% 4) Exit codes:
%      - 0 success
...
try
    ...
    % Headless Brainstorm start
    if ~brainstorm('status')
        brainstorm server local;
    end

    % Import DB + select protocol
    db_import(bstDB);
    ...
    % Check plugin/process availability
    ...
    % Run FOOOF (Specparam): parameters unchanged
    ...
    % Variable tag from env (default keeps backward compatibility)
    tag = getenv_default('FOOOF_TAG', 'post-s100-FOOOF');
